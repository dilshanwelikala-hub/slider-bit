...
- `index.html` — the Slider Bit configurator UI. Tweak every option against a live preview built from placeholder slides (no upload needed), then copy a ready-to-paste embed block containing the options JSON, links to the hosted engine, and an example slide block to replace with your own images or a CMS Collection List. Optionally save the current settings under a short ID ("Save config for reuse") so multiple pages can share them, and load a saved config back into the controls later. A **Presets** panel offers 5 one-click starting configs — Hero/Gallery, Product Grid Carousel, Testimonial/Quote Slider, Logo/Brand Strip, and Thumbnail-Nav Gallery — each just applies a full config (and, for Thumbnail-Nav, a synced companion thumbnail-strip slider via the existing `sync()`/`data-sliderbit-sync` feature) to the normal controls, so presets stay in sync with the engine automatically rather than being a separate code path.
